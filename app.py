...
def _m_priority_flood_fill(dem: np.ndarray, valid: np.ndarray, eps: float = 0.001) -> np.ndarray:
    """Rellena depresiones internas antes de calcular D8.

    Sin este paso, una cuenca grande puede quedar artificialmente cortada en
    pequeños bolsillos/sumideros del DEM y el resultado se subdimensiona.
    """
    h, w = dem.shape
    filled = dem.astype("float32").copy()
    ok = valid & np.isfinite(filled)
    if not np.any(ok):
        raise RuntimeError("DEM sin celdas válidas para relleno hidrológico.")
    visited = np.zeros((h, w), dtype=bool)
    heap = []

    # Borde exterior y celdas válidas adyacentes a nodata actúan como salida hidrológica.
    ...
    for dr, dc in neigh:
        ...
        rr0 = max(0, dr); rr1 = h - max(0, -dr)
        cc0 = max(0, dc); cc1 = w - max(0, -dc)
        border[r0:r1, c0:c1] |= invalid[rr0:rr1, cc0:cc1]

    for r, c in np.argwhere(border & ok):
        visited[int(r), int(c)] = True
        heapq.heappush(heap, (float(filled[int(r), int(c)]), int(r), int(c)))
    ...
    while heap:
        ...
        for dr, dc in neigh:
            ...
            if rr < 0 or rr >= h or cc < 0 or cc >= w:
                continue
            if visited[rr, cc] or not ok[rr, cc]:
                continue
            visited[rr, cc] = True
...
def _m_flow_d8(dem: np.ndarray, valid: np.ndarray):
    """Dirección de flujo D8 sobre DEM hidrológicamente corregido."""
    h, w = dem.shape
    filled = _m_priority_flood_fill(dem, valid)
    ...
    for r in range(h):
        for c in range(w):
            if not valid[r,c] or not np.isfinite(filled[r,c]):
                continue
            z = float(filled[r,c])
            best = None
            for dr, dc, dist in neigh:
                rr, cc = r + dr, c + dc
                if 0 <= rr < h and 0 <= cc < w and valid[rr,cc] and np.isfinite(filled[rr,cc]):
                    dz = z - float(filled[rr,cc])
                    if dz > 0:
                        s = dz / dist
...
def _m_delineate_single_basin(pc: Point, name: str, dem: np.ndarray, transform, valid: np.ndarray, rec: np.ndarray, acc: np.ndarray, snap_radii: List[float], axis: Optional[LineString], axis_buffer_m: float):
    """Delimita cuenca escogiendo la salida más robusta.

    Corrección importante: no se selecciona la primera cuenca de alta confianza,
    porque eso puede escoger una microcuenca cercana al punto y subdimensionar el
    resultado. Se generan candidatos por radio y se privilegia mayor acumulación/
    área aguas arriba, manteniendo advertencias de borde DEM y geometría.
    """
    ...
    for radius in snap_radii:
        try:
            ...
            if touches:
                score -= 30
                warnings.append("La cuenca toca borde DEM: aumentar margen de descarga antes de aceptar el resultado.")
            if dist > 0.8 * radius:
                score -= 10
                warnings.append("El ajuste al drenaje quedó cerca del límite del radio de búsqueda.")
            if shape_idx > 4.5:
                score -= 10
                warnings.append("Polígono alargado: revisar eje/punto de control.")
            if area_km2 < 0.2:
                score -= 35
                warnings.append("Área muy pequeña: posible microcuenca o sumidero local.")
            axis_frac = None
            if axis is not None and axis.length > 0:
                axis_frac = axis.intersection(poly).length / max(axis.length, 1.0)
                if axis_frac < 0.30:
                    score -= 15
                    warnings.append("Baja fracción del eje dentro de la cuenca delimitada.")
            sx, sy = _m_rowcol_to_world(transform, r, c)
            cand = {
                "name": name,
                "polygon": poly,
                "mask": mask,
                ...
                "acc_cells": float(acc_cells),
                "touches_edge": bool(touches),
                "confidence_pct": float(max(0, min(100, score))),
                "shape_index": float(shape_idx),
                "area_km2": float(area_km2),
                "axis_fraction": None if axis_frac is None else float(axis_frac),
                "snapped_x": sx,
                "snapped_y": sy,
                "warnings": warnings,
            }
    ...
    # Selección robusta anti-subdimensionamiento:
    # 1) mayor acumulación en salida, 2) mayor área aguas arriba, 3) mayor confianza.
    # Esto reproduce mejor el criterio práctico de punto de control en cauce principal.
    candidates.sort(key=lambda c: (math.log1p(c.get("acc_cells", 0.0)), c.get("area_km2", 0.0), c.get("confidence_pct", 0.0)), reverse=True)
    best = candidates[0]
    best["candidatos_revision"] = [
        {
            "radio_m": c["snap_radius_m"],
            "area_km2": c["area_km2"],
            "acc_cells": c["acc_cells"],
            "confianza_pct": c["confidence_pct"],
            "toca_borde_dem": c["touches_edge"],
            "dist_snap_m": c["snap_distance_m"],
            "fraccion_eje": c["axis_fraction"],
        }
        for c in candidates
    ]
    return best
...
def run_morfometria_cuenca_module():
    ...
    snap_text = st.sidebar.text_input("Radios ajuste al drenaje [m]", value="300,600,1200,2500,5000,10000", key="m_snap")
    axis_buffer = st.sidebar.number_input("Buffer eje para ajuste de PC [m]", min_value=50.0, max_value=20000.0, value=3000.0, step=50.0, key="m_axis_buffer")
    ...
    tab_img, tab_a, tab_diag, tab_b, tab_c = st.tabs(["Imagen cuenca delimitada", "Curvas hipsométricas", "Diagnóstico de delimitación", "Base técnica JSON", "Descargas"])
    with tab_img:
    ...
    with tab_a:
        ...
        st.pyplot(fig)
    with tab_diag:
        st.markdown("Comparación de candidatos generados por radio de ajuste. La versión corregida prioriza mayor acumulación y mayor área aguas arriba para evitar cuencas subdimensionadas.")
        diag_rows = []
        for etiqueta, basin in [("Cuenca soporte", result["basin_support"]), ("Subcuenca hidrológica", result["basin_hydro"] )]:
            for row in basin.get("candidatos_revision", []):
                d = {"cuenca": etiqueta}
                d.update(row)
                diag_rows.append(d)
        if diag_rows:
            ...
            st.info("No hay diagnóstico de candidatos disponible.")
        st.warning("Si la cuenca toca el borde del DEM o el área sigue siendo menor a la esperada, aumenta el margen DEM y/o los radios de ajuste. No aceptes una cuenca subdimensionada solo porque la app la pudo polygonizar.")
